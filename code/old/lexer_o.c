...
// char getNextChar(FILE * F, char * buffer, int * buffer, int * fwd, int bufSize)
// {
// 	int b = *buffer;
// 	int f = *fwd;
// 	if(verbose==1)
...
dt_token getNextToken(FILE * inputFile, char * buffer, int * buffer, int bufSize)
{
	//if(verbose==1)
	...
	while(shouldRun)
	{
		if(buffer=='\0')
			shouldRun=0;;	
		// input = getNextChar(inputFile, buffer, buffer, &fwd, bufSize);
		input  = *buffer;
		printf("loop: s=%d input=%c ascii=%d line=%d\n",s,input,(int)input,line_number);
		...
		else
		{
			switch(s)
			{
				...
				case 4: // FINAL
				{
					dt_str lexeme = strslice(buffer, *buffer, *buffer + fwd -1);
					s = 1;
					*buffer += (fwd - 1);
				...
				case 44:
				{
					...
					else
					{
						...
						s = 1;
						*buffer += (fwd - 1);
						printf("BEGINVALUEWHILERETURNING: %d", *buffer);
